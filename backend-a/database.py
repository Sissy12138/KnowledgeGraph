...
def import_papers(database_path: Path, papers: list[PaperRecord]) -> int:
    ...
    with connection:
        for paper in papers:
            ...
            if existing and existing[1] == paper.source_updated_at:
                continue
            ...
            for author in paper.authors:
                connection.execute(
                    ...
                    ON CONFLICT(id) DO UPDATE SET
                        ...
                        updated_at = CURRENT_TIMESTAMP
                    """,
                    (author.id, author.source_creator_id, author.name, author.first_name, author.last_name),
                )
                connection.execute(
                    """
            ...
            connection.execute("DELETE FROM paper_attachments WHERE paper_id = ?", (paper_id,))
            connection.executemany(
                """
                ...
                """,
                [
                    (attachment.id, paper_id, attachment.path, attachment.content_type)
