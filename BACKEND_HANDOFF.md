...
- 无需开发环境：双击 `preview/20260831_yanzhitu_frontend_mock_preview_v01.html`。
- 完整开发模式：运行 `npm ci`，再运行 `npm run dev`。
...
| --- | --- | --- | --- |
| 文献 | `src/features/papers/paper.mock.ts`、`paper-detail.mock.ts` | `paper.types.ts` | `paper.service.ts` |
| 知识图谱 | `src/features/graph/graph.mock.ts` | `graph.types.ts` | `graph.service.ts` |
| 证据候选审核 | `src/features/suggestions/suggestion.mock.ts` | `suggestion.types.ts` | `suggestion.service.ts` |
| 分析任务 | `src/features/analysis/analysis-job.mock.ts` | `analysis-job.types.ts` | `analysis-job.service.ts` |
